...
and anything else that speaks MCP: graph, git, docs, decisions, and eleven MCP tools
behind one index. See [the canonical surface](#the-eleven-mcp-tools). Most tools are built around data entities (one
...
| `get_index_status(mode?, path?)` | Whether source-search results are trustworthy, stale, or unknown; path mode explains exact lane inventory and eligibility. |
