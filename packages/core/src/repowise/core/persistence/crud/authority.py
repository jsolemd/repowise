...
def _scope_fields(record: DecisionRecord) -> dict[str, Any]:
    # Lane counts deliberately leave bodies unloaded; scope reads must not
    # trigger async lazy loading through the complete decision adapter.
    return {
        "affected_files": record.affected_files_json,
        "affected_modules": record.affected_modules_json,
        "kind": record.kind,
...
def names_a_scope(record: DecisionRecord) -> list[str]:
    ...
    return named_scope(_scope_fields(record))


def _record_scope(record: DecisionRecord) -> list[str]:
    return record_scope(_scope_fields(record))


def _is_repo_wide(record: DecisionRecord) -> bool:
    return is_repo_wide(_scope_fields(record))
