...
class ASTParser:
    ...
    def _extract_symbols(
        ...
        # Upstream also folds ``cpp_export_type_parents`` into a frozenset for its
        # callable-ancestor skip. This fork does not run that skip — nested locals
        # are extracted and then classified by lexical parent (see
        # ``_link_named_symbol_parents``), which is what gives a nested definition
        # its ``path::Outer::inner`` chain — so the set has no consumer here.
        ...
        for capture_dict in matches:
            ...
            # shape as a top-level one. Upstream reaches this filter after a
            # generic callable-ancestor skip; this fork removed that skip so
            # nested locals can be classified by lexical parent instead. That
            # pass cannot help here: what the query captures is the binding's
            # LEFT-HAND side, which is a sibling of the enclosing binding's
            # body rather than a descendant of it, so the AST-ancestry walk in
            # ``_finalize_symbol_parentage`` finds no owner and the nested
            # ``let`` would surface as a public top-level symbol. Dropping it
            # is what the rest of the fork's languages get from parentage.
            # A ``let`` inside a type body is a field and stays.
        ...
        # builds each symbol id twice in one file. Filtered in parallel with
        # ``symbol_nodes``/``node_types`` for the same reason the pascal
        # branch above is: ``_finalize_symbol_parentage`` zips the three
        # strictly, so a dedupe that shortens only ``symbols`` would raise.
        if file_info.language == "objectivec":
            deduped = _dedupe_objc_interface_symbols(symbols, node_types, objc_container_kinds)
            retained = {id(symbol) for symbol in deduped}
            symbol_nodes = [
                node
                for symbol, node in zip(symbols, symbol_nodes, strict=True)
