...
async def get_savings(
    ...
    days: int | None = Query(None, ge=0, le=3_650, description="Window in days; omit for all time"),
    session: AsyncSession = Depends(get_db_session),
) -> SavingsResponse:
    ...
    with closing(sqlite3.connect(":memory:")) as empty_conn:
        usage = tracking.mcp_usage_summary(empty_conn)
    if db_path.is_file():
        with closing(
            sqlite3.connect(f"file:{db_path.as_posix()}?mode=ro", uri=True, timeout=1)
        ) as conn:
