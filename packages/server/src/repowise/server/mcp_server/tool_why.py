...
async def _why_path(query: str, repo: str | None) -> dict:
    ...
    async with get_session(ctx.session_factory) as session:
        ...
        origin_story = _build_origin_story(query, git_meta, governing + candidates + retired)
    ...
    for entry in target_context.values():
        ...
        if isinstance(rows, list) and rows:
            ...
            if kept:
                ...
                entry["candidate_decisions_omitted"] = recovery_note(len(rows), recall)
        ...
        if isinstance(origin, dict):
            ...
            if isinstance(linked, list) and linked:
                kept = [d for d in linked if clears_floor(score(json.dumps(d, default=str)))]
                if kept:
                    origin["linked_decisions"] = kept
                else:
                    origin.pop("linked_decisions", None)
                    origin["linked_decisions_omitted"] = recovery_note(len(linked), recall)
...
def _focus_archaeology(arch: dict[str, Any], score: Any, recall: str) -> None:
...
async def _why_no_match(
    ...
    *before* the semantic lookup: a nearest-neighbour search over a 614-record
    ...
    if served:
        ...
        result.update(redirect_when_served(served, f"get_why(targets={json.dumps(targets)})"))
...
async def _load_corpus(repo: str | None, targets: list[str] | None) -> tuple:
    ...
    async with get_session(ctx.session_factory) as session:
        ...
        accepted = await accepted_decision_ids(session, repository.id, governing_only=True)
...
async def _why_targets(targets: list[str], repo: str | None) -> dict:
    ...
    ctx, repository, all_decisions, target_git, accepted = await _load_corpus(repo, targets)
    collector = OmissionCollector("get_why", repo_root=ctx.path)
    result_data = {
        "mode": "path",
        "paths": targets,
...
async def _why_search(
    ...
    *,
    ...
    if not supporting:
        ...
        decisions = _merge_decisions(primary, [], {}, collector, accepted)
        candidates = [
    ...
    if episodes and targets:
        ...
        kept = [e for e in episode_population if clears_floor(score(json.dumps(e, default=str)))]
