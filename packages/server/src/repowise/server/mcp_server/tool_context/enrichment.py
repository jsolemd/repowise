    ...
    for edge_type, edges in edges_by_type.items():
        ...
        for direction, rows in (
            ...
            cap_collection(
                ...
                collector,
                label=(
                    f"{target} :: relations.{edge_type}.{direction} beyond cap={_RELATION_ROW_CAP}"
                ),
            )
    ...
    if paths:
        try:
            ...
            async with session.begin_nested():
                ...
                stored = bool(rows) or await doc_drift_references_stored(session, repository.id)
            ...
            reason = UNAVAILABLE_NO_TABLE if is_missing_table(exc) else UNAVAILABLE_READ_FAILED
            for name in wanted:
                cards[name]["doc_drift"] = {"unavailable": reason}
            return

    kept = filter_rows_by_attr(rows, "document_path", exclude_spec)
    excluded_by_target = Counter(r.target_path for r in rows) - Counter(r.target_path for r in kept)
...
def _doc_reference_block(
    ...
    *,
    ...
    block: dict[str, Any] = {}
    emitted = collapse_reference_sites([serialize_doc_drift_reference_row(row) for row in rows])
    cap_collection(
        block,
        "references",
        emitted,
