...
def _persist_index_only_update(
    ...
    try:
        ...
        cleanup_retired_page_vectors(repo_path, retired_vector_ids, vector_store=vector_store)
    except Exception as exc:
    ...
    *,
    ...
    try:
        ...
        async with get_session(sf) as session:
            ...
            if reconcile_full_generation and not exclusion_plan.refusals:
                try:
                    from repowise.core.pipeline.persist import (
            ...
            if doc_drift_report is not None:
                try:
                    ...
                    with timed(timings, "persist.doc_drift"):
                        await replace_doc_drift_guarded(session, repo_id, doc_drift_report)
                except Exception as exc:
        ...
        # SQL committed above. Persist the ids before touching either external
        # index: swept rows cannot reveal their ids again on a later retry.
        vector_cleanup_ids = (
        ...
        try:
            ...
            async with get_session(sf) as session:
                ...
                if coverage_authoritative:
                    ...
                    # working tree, and the two diverge when the tree moved
                    # after the last index (issue #1747). Live HEAD is the
                    # provenance answer: it is the tree the coverage was
                    # measured against.
