"""Migration solemd_0003 separates parent display text from exact symbol identity."""
...
def test_migration_0057_upgrade_and_downgrade_preserve_parent_semantics(
    ...
    _run_migration(db_path, "solemd_0002")
    _seed_legacy_rows(db_path)

    _run_migration(db_path, "solemd_0003")
    with sqlite3.connect(db_path) as conn:
    ...
    _run_migration(db_path, "solemd_0002", downgrade=True)
    with sqlite3.connect(db_path) as conn:
