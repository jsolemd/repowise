...
async def test_a_repo_wide_candidate_never_takes_the_whole_lane(tmp_path, monkeypatch):
    ...
    assert block is not None
    lines = block.splitlines()
    assert sum("Global candidate" in ln for ln in lines) <= decision_inject._MAX_CANDIDATE_GLOBALS
...
async def test_a_tier_does_not_reach_through_a_module_link(tmp_path, monkeypatch):
    ...
    assert (
        decision_inject._edit_decision_notice(tmp_path, "src/core/health/engine.py", "s1", {})
        is None
    )
