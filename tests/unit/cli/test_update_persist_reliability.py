...
def test_full_update_vector_cleanup_follows_sql_commit(tmp_path, monkeypatch, rollback):
    """Swept ids retry after vector failure, and rollback never deletes vectors."""
    ...
    (tmp_path / ".repowise").mkdir()
    retired = sorted(RETIRED_IDS)[0]
    asyncio.run(_seed_pages(tmp_path, "repo", [retired]))
    vectors = SimpleNamespace(
        _embedder=None,
        delete_many=AsyncMock(side_effect=OSError("vector store unavailable")),
    )

    async def persist():
        return await _persist_full_update_async(
            repo_path=tmp_path,
            repo_name="repo",
            generated_pages=[],
            file_diffs=[],
            ...
            knowledge_graph_result=None,
            degraded=[],
            require_config_rebuild_success=rollback,
        )

    if rollback:
        # The absent builder deliberately degrades a required derived-store
        # write after the sweep, causing the configuration transaction to fail.
        with pytest.raises(RuntimeError, match="Configuration rebuild"):
            asyncio.run(persist())
        assert retired in asyncio.run(_page_ids(tmp_path))
        vectors.delete_many.assert_not_awaited()
        assert load_cleanup_debt(tmp_path)["vectors"] == set()
        return

    asyncio.run(persist())
    assert retired not in asyncio.run(_page_ids(tmp_path))
    assert load_cleanup_debt(tmp_path)["vectors"] == {retired}

    vectors.delete_many = AsyncMock()
    asyncio.run(persist())
    vectors.delete_many.assert_awaited_once_with([retired])
    assert load_cleanup_debt(tmp_path)["vectors"] == set()
