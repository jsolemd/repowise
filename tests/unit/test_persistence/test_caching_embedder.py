...
async def test_provider_failure_is_retried_and_recovery_can_be_cached():
    class _Recovering(_CountingEmbedder):
        async def embed(self, texts):
            if not self.calls:
                self.calls.append(list(texts))
    ...
    recovered = await embedder.embed(["q"])
    assert await embedder.embed(["q"]) == recovered
