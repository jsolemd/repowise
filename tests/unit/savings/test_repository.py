...
def test_fork_legacy_distill_rows_survive_schema_upgrade(tmp_path):
    db_path = tmp_path / "omissions.db"
    _seed_legacy(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute("INSERT INTO savings(created_at, filter, source, command, raw_tokens, distilled_tokens) VALUES (3, 'logs', 'cli', 'command', 100, 10)")
    with OmissionStore(db_path) as store:
        assert store._conn.execute("SELECT source, raw_tokens, distilled_tokens FROM savings").fetchall() == [("cli", 100, 10)]
